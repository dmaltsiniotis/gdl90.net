...
namespace GDL90.Tests {
    public class TrafficReportTest
    {
        ...
        */
        ...
        [Fact]
        public void TrafficReport_Parses_Example_Message()
        {
            TrafficReport tf = new TrafficReport(Message.AppendFlagBytes(Message.AppendCRC(TrafficReportExample)));

            Assert.True(tf.ValidCRC);
            Assert.Equal(TrafficReport.TrafficAlertStatusEnum.No_alert, tf.TrafficAlertStatus);
            ...
            Assert.Equal(TrafficReport.HeadingTypeEnum.TrueTrackAngle, tf.HeadingType);
            Assert.Equal(TrafficReport.AirGroundStateEnum.Airborne, tf.AirGroundState);
            Assert.Equal(TrafficReport.TrafficReportUpdateEnum.ReportUpdated, tf.TrafficReportUpdate);
